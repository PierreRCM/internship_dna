...
def ajouter_monomere(matrice, pos_last, number):
    ...
    collision = test_collision(new_pos, matrice)
    matrice[new_pos[0], new_pos[1], new_pos[2]] = number

    return matrice, new_pos, collision
...
def test_collision(new_pos, mat):

    if mat[new_pos[0], new_pos[1], new_pos[2]] != 0:
    ...
    for j in range(essaie):
        ...
        while colli:
            ...
            for i in range(taille_polymere):
                ...
                reseau, new_pos, colli = ajouter_monomere(reseau, new_pos, i)
                if colli:
                    break
